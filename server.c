...
int sendFile(FILE *fd){
	...
	int readLength = 0, sendbytes = 0;

	int seq_num = 0, sended_num = 0, is_last=0;
	//adjust the writing head back to zero
	fseek(fd,0,SEEK_SET);
	//init. my_snd_pkt and my_rcv_pkt 
	Udp_pkt my_snd_pkt, my_rcv_pkt;
	my_rcv_pkt.header.ack_num = 0;
	my_rcv_pkt.header.seq_num = 0;
	my_rcv_pkt.header.isLast = 0;
	bzero(my_rcv_pkt.data, sizeof(my_snd_pkt.data));
	my_snd_pkt.header.ack_num = 0;
	my_snd_pkt.header.seq_num = 0;
	my_snd_pkt.header.isLast = 0;
	...
	while (!receiveClientSayIsLast) {
		...
		while (plzResend) {
			...
			//==========================
			int tmpsend = 0;
			// <= 0 means fail to send or send a packet with size 0
			if ((tmpsend = sendto(sockfd, &my_snd_pkt, sizeof(my_snd_pkt), 0,(struct sockaddr *)&client_info, len)) <= 0){
        	    printf("Send packet Failed (%d)!\n", tmpsend);
        	}
			printf("Send packet #%d ( %d bytes) success", seq_num, tmpsend);

			//======================================
			// Checking timeout & Receive client ack
			//======================================
			int exp_rcv_pkt_within = (clock()*1000)/CLOCKS_PER_SEC + TIMEOUT, tmprcv = 0;
			while ((clock()*1000)/CLOCKS_PER_SEC < exp_rcv_pkt_within) {
				...
				if(((tmprcv = recvfrom(sockfd, &my_rcv_pkt, sizeof(my_rcv_pkt), 0, (struct sockaddr *)&info, (socklen_t *)&len)) > 0)){
					if(my_rcv_pkt.header.ack_num == seq_num){
						...
						seq_num++;
						bzero(my_snd_pkt.data, sizeof(my_snd_pkt.data));
						printf("Receive a packet ack_num = %d and the correspond data size is %d byte\n",my_rcv_pkt.header.ack_num, readLength);
						break;
					}
				}
			}
			if(plzResend){
				printf("snd pkt #%d ( %d bytes) meets TIMEOUT!", seq_num, readLength);
			}
		}
	}
